...
## 2. test density function approxfun() to make sure it makes a density plot similar to actual
```{r, test-approxfun}
...
## 2. Calculate the area under the curve of ach animal's density plots for epidermis and dermis
```{r, calc-integral}
epidermis_auc <- calculate_auc(all_epidermis_lengths_in_microns)
dermis_auc <- calculate_auc(all_dermis_lengths_in_microns)
```

## 3. Label integrals with animal and treatment
### 3a. Write funciton to unblind animal identities found in auc df's
```{r, unblind-fnc}
...
### 3b. make integral dataframe with animal names and treatment groups 
```{r, label-integrals}
...
labelled_dermis_auc$treatment <- add_identity_unblind_column(labelled_dermis_auc)
labelled_epidermis_auc$treatment <- add_identity_unblind_column(labelled_epidermis_auc)
```

## 1. create density plots for each animal's EPIDERMIS lengths...
