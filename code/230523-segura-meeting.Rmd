...
### exclude "problem" Animals
See oneNote "4/3/23 Progress Update:" for explanation (Animals A,F,I,X,Z10)
```{r, exclude-bad-animals}
...
##### zoom in on select graphs
Only one graph example is shown
```{r warning=FALSE}

# plot using ggplot and facet_wrap()
taregtAnimal_only <- long1[] %>% filter(startsWith(name, "Animal_F"))

p1<-ggplot(taregtAnimal_only, aes(x = value)) +
  ...
  theme(axis.title.x = element_text())
  ggsave("../visualizations/shape-center-spread/epiderm/Animal_F-micron-length-spread-zoomedView.jpg", plot=p1, dpi = 300, width = 6, height = 4, units = "in")

p2<-ggplot(taregtAnimal_only, aes(x = value)) +
  ...
  theme(axis.title.x = element_text())
  ggsave("../visualizations/shape-center-spread/epiderm/Animal_F-micron-length-spread-standardView.jpg", plot=p2, dpi = 300, width = 6, height = 4, units = "in")
