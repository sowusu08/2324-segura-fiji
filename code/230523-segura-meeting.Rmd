...
# plot using ggplot
# variables that chaneg are `long*[]` and `Animal_*`
targetAnimal_only <- long5[] %>% filter(startsWith(name, "Animal_Z9."))
...
p1<-ggplot(targetAnimal_only, aes(x = value)) +
  ...
  theme(axis.title.x = element_text())
  ggsave("../visualizations/shape-center-spread/derm/Animal_Z9-micron-length-spread-zoomedView.jpg", plot=p1, dpi = 300, width = 6, height = 4, units = "in")
...
p2<-ggplot(targetAnimal_only, aes(x = value)) +
  ...
  facet_wrap(~ name, scales = "fixed") +
  labs(x = "Micron Value", y = "Frequency")+
  scale_x_continuous(limits = c(0,197), expand = c(0, 0))+
  theme(axis.title.x = element_text())
  ggsave("../visualizations/shape-center-spread/derm/Animal_Z9-micron-length-spread-standardView.jpg", plot=p2, dpi = 300, width = 6, height = 4, units = "in")
